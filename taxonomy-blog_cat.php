...
<section id="wrapperArchiveContents">
	...
	<div id="wrpperPankuzu">
		...
		<a href="<?php echo get_post_type_archive_link( 'blog' ); ?>"><?php echo get_post_type_object('blog')->label; ?></a>
		<?php // 表示しているタームアーカイブの親・先祖タームの情報を取得して表示
		  $term_id = get_queried_object_id(); // タームのIDを取得
		  $ancestors = get_ancestors($term_id, 'blog_cat'); // タクソノミースラッグを指定してタームの配列を取得
		  $ancestors = array_reverse($ancestors); // 子親の順番で表示されるので、親子の順番に変更
		  foreach( $ancestors as $ancestor ) { // 配列から個々の値を取り出す
			$parent_term = get_term($ancestor, 'blog_cat'); // タームIDとタクソノミースラッグを指定してターム情報を取得
			echo '<a href="'.site_url().'/blog/blog_cat/'.$slug = $parent_term->slug.'">'.$name = $parent_term->name.'</a>'; // タームスラッグを取得
		  }
		?>
